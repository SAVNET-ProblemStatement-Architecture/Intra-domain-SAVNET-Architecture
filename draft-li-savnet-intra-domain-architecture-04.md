...
This document proposes the intra-domain SAVNET architecture. Routers in the intra-domain network can generate SAV rules based on not only local routing information but also SAV-specific information. The architecture supports more accurate validation than the mechanisms that rely on only local routing information for SAV. This is because SAV-specific information provides the information that is missing from local routing information but is necessary to generate accurate SAV rules. 
Routers can communicate with each other to automatically collect SAV-specific information and update SAV rules. 

The document primarily introduces SAV-specific information, architectural components, and the SAV-specific information communication mechanism between them. The future intra-domain SAV mechanisms/protocols can be developed based on the architecture. Concrete protocol designs, extensions or implementations are not the focus of this document. 
...
There are many intra-domain SAV mechanisms available such as ACL-based filtering [RFC2827], strict uRPF [RFC3704], and loose uRPF [RFC3704]. SAV rules are generated by either routing information or manual configurations. However, they are faced with the problems of high operational overhead and inaccurate validation in some scenarios {{I-D.ietf-savnet-intra-domain-problem-statement}}. Some requirements of future intra-domain mechanisms are proposed in {{I-D.ietf-savnet-intra-domain-problem-statement}}, i.e., automatic update, accurate validation, working in incremental/partial deployment, fast convergence, and necessary security guarantee. 

An intra-domain architecture is proposed in this document to meet the above requirements. Consider that routers only relying on local routing information cannot generate accurate SAV rules in some scenarios. SAV-specific information is defined which supplements the SAV-related information missing from local routing information. Routers in intra-domain networks can communicate SAV-specific information and automatically generate accurate SAV rules. 

The architecture can work well when only part of edge/border routers are upgraded and thus supports good incremental/partial deployment. When SAV-specific information is not fully available, routers can leverage locally available SAV-related information to generate SAV rules while avoiding improper block problems. 

The architecture also supports good convergence. SAV-specific information can be propogated with the similar rate (or speed) of routing information. Therefore, when routing information changes, routers can timely sense the update of SAV-specific information and update SAV rules. 

In many cases, an intra-domain network is under the same administration and can be considered as a trusted domain. The document provides some security considerations of the proposed architecture. 
...
Local Routing Information: The locally available RIB or FIB information that can also be used for SAV rule generation besides the routing purpose. 

SAV-specific Information: The information specialized for SAV that cannot be inferred from local routing information but is necessary for generating accurate SAV rules. For example, the source prefixes or incoming directions of source prefixes that are missing from local routing information belong to SAV-specific information. 

SAV-related Information: Any information that is useful for generating SAV rules. Generally, SAV-related information includes local routing information and SAV-specific information. 

SAV-specific Information Communication Mechanism: The mechanism for communicating SAV-specific information. The mechanism can be a new protocol or an extension to existing protocols. 

SAV Information Base: A table or data structure for storing SAV-related information. 

False Positive: The validation results that the packets with legitimate source IP addresses are considered invalid due to inaccurate SAV rules. False positive may induce improper block problems if routers are configured to block the "invalid" packets. 

False Negative: The validation results that the packets with spoofed source IP addresses are considered valid due to inaccurate SAV rules. False negative may induce improper permit problems if routers forward the "valid" packets normally. 
